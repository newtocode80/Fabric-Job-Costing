...
labour_cost_total            filter_aggregate      1  pass    7/7
...
cost_by_job_type             join                  1  warn    8/8
                                                              ! numbers from the prompt, not this result: 40
...
cost_by_region               refusal               0  pass    6/6
profit_by_job                required_pattern      1  warn    11/11
                                                              ! numbers from the prompt, not this result: 71676.06
biggest_jobs                 ambiguous             0  pass    6/6
---------------------------------------------------------------------------------------
14/15 passed (4 with warnings)
x = failed check    ! = number printed in the prompt but not in this result
```

### The one failing case is the rule working

`approved_change_order_value` answers correctly — $71,676.06 — and then adds *"the
remaining **19** change orders (**11** Pending, **8** Rejected)"*. 11 and 8 are
quoted from `dim_change_order.Status`'s own description and warn. **19 is their sum**:
computed, not read, and it fails.

Nothing about that sentence is wrong. It fails because the checker cannot tell
quoting from computing — in the text they are identical — and the alternative is
worse. The failure this whole check exists for was built the same way:

> *"The remaining completed jobs (52 total minus the 8 above = 44 assessable) had
> 33 late finishes — meaning only 11 completed jobs finished on time or early."*

Every figure there traces to the prompt or to a count done in the model's head, and
every one of them is wrong in a way a reader would not notice. Permit arithmetic on
prompt figures and that passes silently; forbid it and a harmless sentence about
change orders fails. **The trade is deliberate**, and this row is what it costs: one
conservative flag, visible, on an answer that is otherwise correct.

Left as a failure rather than suppressed. A green board that hides the trade-off
would be worth less than an amber one that shows it.
